...
func (proof *RangeProof) Unmarshal(tree []byte) error {
	cdc := amino.NewCodec()
	return cdc.UnmarshalBinary(tree, proof)
}

func (proof *RangeProof) MarshalTo(tree []byte) (int, error) {
	cdc := amino.NewCodec()
	proofBytes, err := cdc.MarshalBinary(proof)
	if err != nil {
		return 0, err
	}
...
func (proof *RangeProof) Size() int {
	cdc := amino.NewCodec()
